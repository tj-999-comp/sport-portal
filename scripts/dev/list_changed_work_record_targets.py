...
def changed_targets(before: str, after: str, *, publish_only: bool = False) -> list[str]:
    ...
    result = subprocess.run(
        ["git", "diff", "--name-only", "--diff-filter=ACMRTUXB", before, after, "--", "work-records"],
        cwd=ROOT, check=True, capture_output=True, text=True,
    )
    targets = sorted({match.group(1) for path in result.stdout.splitlines() if (match := TARGET_PATH_RE.fullmatch(path))})
    if not publish_only:
        return targets
    return [target for target in targets if is_publishable(target)]
