...
function renderMatches(matches = []) {
  ...
  dates.forEach((date) => {
    ...
    dayMatches.sort((a, b) => (a.kickoff || '').localeCompare(b.kickoff || '')).forEach((match) => {
      ...
      card.innerHTML = `<div class="match-meta"><span>第${escapeHtml(match.matchday || '―')}節</span><span class="${finished ? 'match-time' : 'match-state'}">${escapeHtml(stateText)}</span></div><div class="scoreline"><div class="team-row home${homeWon ? ' winner' : awayWon ? ' loser' : ''}"><div class="team-name-block"><span class="team-side-label">Home</span><span class="team-name">${escapeHtml(match.home?.short || match.home?.name)}</span></div><span class="score${finished ? '' : ' score-placeholder'}">${finished ? escapeHtml(match.homeScore) : '0'}</span></div><span class="score-divider" aria-hidden="true">—</span><div class="team-row away${awayWon ? ' winner' : homeWon ? ' loser' : ''}"><span class="score${finished ? '' : ' score-placeholder'}">${finished ? escapeHtml(match.awayScore) : '0'}</span><div class="team-name-block"><span class="team-side-label">Away</span><span class="team-name">${escapeHtml(match.away?.short || match.away?.name)}</span></div></div></div>`;
      list.append(card);
    });
    root.append(day);
  });
